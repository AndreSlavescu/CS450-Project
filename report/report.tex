...
Serving large language models (LLMs) with low latency is a central challenge in deploying interactive AI systems. In single-sequence inference, the decode phase generates one token per forward pass, and each pass executes hundreds of small CUDA kernels across all model layers. The resulting launch overhead, synchronization stalls, and unnecessary global memory round-trips dominate execution time, leaving the GPU underutilized. Production engines like vLLM~\cite{vllm} and SGLang~\cite{sglang} mitigate this through techniques such as paged attention and structured generation, but they still rely on the standard multi-kernel execution model, which imposes a hard floor on per-token latency.
