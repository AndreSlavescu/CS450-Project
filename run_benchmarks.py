...
def run_benchmark(gpu, engine):
    ...
    if process.returncode != 0:
        print(f"Error running {engine} on {gpu} (return code {process.returncode})", flush=True)
        return None
        
    if throughput is None:
        print(f"Could not parse throughput from output for {engine} on {gpu}.", flush=True)
    ...
    for gpu in gpus:
        for engine in engines:
            for run_idx in range(num_runs):
                print(f"--- {gpu} | {engine} | Run {run_idx+1}/{num_runs} ---", flush=True)
                ...
                if throughput is not None:
                    ...
                    print(f"Parsed Throughput: FAILED", flush=True)
