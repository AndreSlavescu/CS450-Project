...
def run_benchmark(gpu, engine, run_idx, num_runs):
    ...
    if process.returncode != 0:
        safe_print(f"{prefix} Error (return code {process.returncode})", flush=True)
        return gpu, engine, throughput
        
    if throughput is None:
        safe_print(f"{prefix} Could not parse throughput.", flush=True)
    else:
        safe_print(f"{prefix} Finished. Parsed Throughput: {throughput}", flush=True)
        ...
        for gpu in gpus:
            for engine in engines:
                for run_idx in range(num_runs):
                    futures.append(executor.submit(run_benchmark, gpu, engine, run_idx, num_runs))
