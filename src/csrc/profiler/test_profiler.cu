...
__global__ void profiled_kernel(float* output, int n,
                                profiler::event_record* g_events,
                                int* g_counts) {
    __shared__ profiler::block_state prof;
    __shared__ float smem[256];

    if (threadIdx.x == 0) prof.init();
    __syncthreads();

    if (threadIdx.x == 0) prof.record(EV_INIT);
    ...
    if (threadIdx.x == 0) prof.record(EV_INIT_END);

    if (threadIdx.x == 0) prof.record(EV_COMPUTE);
    ...
    if (threadIdx.x == 0) prof.record(EV_COMPUTE_END);

    if (threadIdx.x == 0) prof.record(EV_REDUCE);
    ...
    if (threadIdx.x == 0) prof.record(EV_REDUCE_END);
    ...
    __syncthreads();
    if (threadIdx.x == 0) {
        prof.flush(g_events + blockIdx.x * profiler::config::MAX_EVENTS,
                   g_counts + blockIdx.x);
    }
}
...
int main() {
    ...
    profiled_kernel<<<NUM_BLOCKS, BLOCK_SIZE>>>(
        d_output, N, prof_buf.d_events, prof_buf.d_counts);
