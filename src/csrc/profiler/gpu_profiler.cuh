...
template <int N> struct region_timer {
    uint64_t begin_ns[N];
    uint64_t end_ns[N];
...
struct host_buffer {
    ...
    __host__ __device__ event_record* block_events(int block_idx) { return d_events + block_idx * config::MAX_EVENTS; }

    __host__ __device__ int* block_count(int block_idx) { return d_counts + block_idx; }
    ...
    std::pair<std::vector<int>, std::vector<event_record>> download() {
        ...
        cudaMemcpy(h_counts.data(), d_counts, num_blocks * sizeof(int), cudaMemcpyDeviceToHost);
        cudaMemcpy(h_events.data(), d_events, num_blocks * config::MAX_EVENTS * sizeof(event_record),
    ...
    void print_report(const event_names* names = nullptr) {
        if (!d_events || !d_counts || num_blocks == 0)
            return;
        ...
        for (int b = 0; b < num_blocks; b++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                printf("%-6d %-8d %-20s %-6d %-18lu %-18lu\n", b, base[i].event_id, name, base[i].sm_id,
                       (unsigned long)base[i].global_ns, (unsigned long)base[i].sm_cycles);
            }
        }
        printf("======================================================================\n");
    }
    ...
    void export_perfetto_json(const char* path, const event_names* names = nullptr, bool paired = true) {
        if (!d_events || !d_counts || num_blocks == 0)
            return;
        auto [h_counts, h_events] = download();
        ...
        std::sort(all.begin(), all.end(),
                  [](const flat_event& a, const flat_event& b) { return a.ev.global_ns < b.ev.global_ns; });
        ...
        for (size_t i = 0; i < all.size(); i++) {
            ...
            fprintf(f,
                    ...
                    ename.c_str(), ph, (unsigned long)ts_us, fe.ev.sm_id, eid, fe.block, (unsigned long)fe.ev.sm_cycles,
