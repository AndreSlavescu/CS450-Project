...
__global__ void qkv_rope_append_kernel(
    ...
    qkv_rope_append_device(q_out, k_out, v_out, k_cache, v_cache,
                           hidden, attn_ln_w, qkv_weight, q_norm_w, k_norm_w,
                           cos_cached, sin_cached, pos_id, max_seq_len,
                           g_events, g_counts);
}

__global__ void oproj_residual_kernel(
    float* hidden_states, const float* __restrict__ attn_out,
...
__global__ void upgate_silu_kernel(
    float* silu_out, const float* __restrict__ hidden,
...
__global__ void downproj_residual_kernel(
    float* hidden_states, const float* __restrict__ silu_out,
...
__global__ void rms_lm_head_kernel(
    float* logits, const float* __restrict__ hidden,
    const float* __restrict__ norm_w, const float* __restrict__ lm_head_w,
    int vocab_size,
    profiler::event_record* g_events, int* g_counts
) {
...
std::vector<torch::Tensor> qkv_rope_append_forward(
    torch::Tensor hidden, torch::Tensor attn_ln_w, torch::Tensor qkv_weight,
    torch::Tensor q_norm_w, torch::Tensor k_norm_w,
    torch::Tensor cos_cached, torch::Tensor sin_cached,
    torch::Tensor k_cache, torch::Tensor v_cache, int pos_id
) {
    ...
    size_t smem_bytes = (HIDDEN_DIM + QKV_DIM + kernels::WARP_SIZE) * sizeof(float)
    ...
    qkv_rope_append_kernel<<<1, block_size, smem_bytes>>>(
        ...
        hidden.data_ptr<float>(), attn_ln_w.data_ptr<float>(),
        qkv_weight.data_ptr<float>(), q_norm_w.data_ptr<float>(),
        k_norm_w.data_ptr<float>(), cos_cached.data_ptr<float>(),
        sin_cached.data_ptr<float>(), pos_id, max_seq_len,
        nullptr, nullptr);
...
torch::Tensor oproj_residual_forward(
    torch::Tensor hidden_states, torch::Tensor attn_out, torch::Tensor o_proj_w
) {
    ...
    oproj_residual_kernel<<<1, block_size, smem_bytes>>>(
        output.data_ptr<float>(), attn_out.data_ptr<float>(),
...
torch::Tensor upgate_silu_forward(
    torch::Tensor hidden, torch::Tensor mlp_ln_w,
    torch::Tensor gate_w, torch::Tensor up_w
) {
    auto silu_out = torch::empty({INTERMEDIATE_DIM}, hidden.options());
    const int block_size = 256;
    size_t smem_bytes = (HIDDEN_DIM + kernels::WARP_SIZE) * sizeof(float)
                        + sizeof(profiler::block_state);

    upgate_silu_kernel<<<1, block_size, smem_bytes>>>(
        silu_out.data_ptr<float>(), hidden.data_ptr<float>(),
...
torch::Tensor downproj_residual_forward(
    torch::Tensor hidden_states, torch::Tensor silu_out, torch::Tensor down_proj_w
) {
    auto output = hidden_states.clone();
    const int block_size = 256;
    ...
    downproj_residual_kernel<<<1, block_size, smem_bytes>>>(
        output.data_ptr<float>(), silu_out.data_ptr<float>(),
...
torch::Tensor rms_lm_head_forward(
    torch::Tensor hidden, torch::Tensor norm_w,
    torch::Tensor lm_head_w, int vocab_size
) {
    ...
    rms_lm_head_kernel<<<1, block_size, smem_bytes>>>(
        logits.data_ptr<float>(), hidden.data_ptr<float>(),
        norm_w.data_ptr<float>(), lm_head_w.data_ptr<float>(),
        vocab_size, nullptr, nullptr);
...
torch::Tensor oproj_residual_forward_profiled(
    ...
    oproj_residual_kernel<<<grid_size, block_size, smem_bytes>>>(
        output.data_ptr<float>(), attn_out.data_ptr<float>(),
        o_proj_w.data_ptr<float>(), prof_buf.d_events, prof_buf.d_counts);
    cudaDeviceSynchronize();
...
torch::Tensor downproj_residual_forward_profiled(
    ...
    downproj_residual_kernel<<<grid_size, block_size, smem_bytes>>>(
        output.data_ptr<float>(), silu_out.data_ptr<float>(),
        down_proj_w.data_ptr<float>(), prof_buf.d_events, prof_buf.d_counts);
    cudaDeviceSynchronize();
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("silu_multiply", &silu_multiply,
          "Fused SiLU-multiply: output = SiLU(gate) * up (vectorized)");
}
