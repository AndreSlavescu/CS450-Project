...
__device__ void rms_lm_head_device(float* logits,                       // [vocab_size] output
                                   ...
                                   const float* __restrict__ lm_head_w, // [vocab_size, QWEN3_1_7B.hidden_size]
...

    {
        ...
        for (int out_base = tid * ILP; out_base < aligned_vocab; out_base += num_threads * ILP) {
            ...
            const float4* row0 =
                reinterpret_cast<const float4*>(lm_head_w + (long long)(out_base + 0) * QWEN3_1_7B.hidden_size);
            const float4* row1 =
                reinterpret_cast<const float4*>(lm_head_w + (long long)(out_base + 1) * QWEN3_1_7B.hidden_size);
            const float4* row2 =
                reinterpret_cast<const float4*>(lm_head_w + (long long)(out_base + 2) * QWEN3_1_7B.hidden_size);
            const float4* row3 =
                reinterpret_cast<const float4*>(lm_head_w + (long long)(out_base + 3) * QWEN3_1_7B.hidden_size);
        ...
        for (int out_idx = aligned_vocab + tid; out_idx < vocab_size; out_idx += num_threads) {
            float acc = 0.0f;
            const float4* row =
                reinterpret_cast<const float4*>(lm_head_w + (long long)out_idx * QWEN3_1_7B.hidden_size);
            for (int j = 0; j < QWEN3_1_7B.hidden_size / 4; j++) {
                float4 x = input4[j];
