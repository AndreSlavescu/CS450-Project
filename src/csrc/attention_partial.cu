 ...
 *
 * For one KV head, processes GQA_RATIO=2 Q heads against the KV cache
 ...
 *
 * Inputs:
 *   q:       [GQA_RATIO, HEAD_DIM] = [2, 128] — the query vectors for this KV head group
 *   k_cache: [seq_len, HEAD_DIM]   — K cache for this KV head
 *   v_cache: [seq_len, HEAD_DIM]   — V cache for this KV head
 *
 * Outputs:
 *   out: [GQA_RATIO, HEAD_DIM] = [2, 128] — attention output
 *   lse: [GQA_RATIO]           = [2]      — log-sum-exp for reduction
 */
__global__ void attention_partial_kernel(
    ...
    if (tid == 0 && has_profiler) prof->record(EV_QK);

    // Online softmax attention for each Q head
    // For each Q head h in [0, GQA_RATIO):
    ...
    for (int h = 0; h < GQA_RATIO; h++) {
        ...
        // Each thread handles HEAD_DIM/num_threads elements
        float acc[4];  // HEAD_DIM/num_threads = 128/256 < 1, so we need different parallelization
        ...
        int my_out_idx = tid < HEAD_DIM ? tid : -1;
        if (my_out_idx >= 0) local_acc[0] = 0.0f;

        for (int pos = 0; pos < seq_len; pos++) {
            const float* k_pos = k_cache + pos * HEAD_DIM;
            const float* v_pos = v_cache + pos * HEAD_DIM;
    ...
    if (tid == 0 && has_profiler) prof->record(EV_QK_END);
